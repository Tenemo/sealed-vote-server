...
export const withTransaction = async <T>(
    ...
    handler: (databaseTransaction: DatabaseTransaction) => Promise<T>,
): Promise<T> => {
    return fastify.database.transaction(async (databaseTransaction) =>
        handler(databaseTransaction as DatabaseTransaction),
    );
};
