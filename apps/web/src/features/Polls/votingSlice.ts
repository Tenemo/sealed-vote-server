...
export const votingSlice = createSlice({
  ...
  reducers: {
    ...
    setKeys: (
      ...
      const { pollId, privateKey, publicKey, commonPublicKey } = action.payload;
    ...
    setVoterSession: (
      ...
      const { pollId, voterName, voterIndex, voterToken } = action.payload;
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addMatcher(
        (action): action is PayloadAction<unknown, string, VoteThunkMeta> =>
          isVoteThunkAction(action, 'pending'),
        (state, action) => {
          const voteState = ensureVoteState(state, action.meta.arg.pollId);
      ...
      .addMatcher(
        (
          action,
        ): action is PayloadAction<string | undefined, string, VoteThunkMeta> =>
