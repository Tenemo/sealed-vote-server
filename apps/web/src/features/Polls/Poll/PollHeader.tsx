...
const PollHeader = ({ poll, pollId }: PollHeaderProps): React.JSX.Element => {
  ...
  return (
    <div className="flex w-full flex-col items-center p-2 text-center">
      <h2 className="mb-2 text-xl font-semibold">Vote: {poll.pollName}</h2>
      {creatorToken && poll.isOpen && (
        <p className="mb-2 max-w-[720px]">
          You are the creator of this vote. When there will be more than 1
          voter, including yourself, you can begin the vote to calculate the
          results and prevent any new votes.
        </p>
      )}
      ...
      {creatorToken && poll.isOpen && (
        <div className="mb-2 flex w-full flex-col items-center justify-center">
          ...
          {closeError && (
            <Alert className="mt-2 w-full max-w-[720px]" variant="destructive">
              <AlertDescription>{renderError(closeError)}</AlertDescription>
            </Alert>
          )}
        </div>
      )}
      ...
      <div className="flex w-full flex-col items-center justify-center">
        {progressMessage && (
          <>
            ...
            <Alert className="mt-2 w-full max-w-[720px]" variant="info">
