...
const Voting = ({ onVote, poll, pollId }: VotingProps): React.JSX.Element => {
  ...
  useEffect(() => {
    setSelectedScoresForm((currentScores) => {
      ...
      return Object.keys(currentScores).length ? currentScores : nextScores;
    });
  ...
  return (
    <div className="w-full">
      <p className="px-2 py-1 text-center">
        Rate choices from 1 to 10. The results will be ranked by geometric mean
        of all votes per item. All voters need to be present in order to
        complete the vote.
      </p>
      <ul className="w-full">
