...
const createPollSocialImagePath = (
    ...
    const pathname = `${pollSocialImagePathPrefix}${encodeURIComponent(pollSlug)}.png`;
...
const createPollPageImageUrl = (
    ...
    pollSlug
        ? createAbsoluteUrl(
              origin,
              createPollSocialImagePath(pollSlug, {
                  isComplete,
              }),
          )
...
export const buildCreatePageSeo = ({
    ...
    return {
        ...
        title: createPollSeoTitle,
        url,
    };
};

const createPollPageDescription = ({
    isComplete,
}: {
    isComplete: boolean;
}): string => {
    return isComplete
        ? pollResultsFallbackDescription
        : pollPageFallbackDescription;
};

export const buildPollPageSeo = ({
    origin,
    pollPath,
    pollSlug,
    resultScores,
    ...
    const description = createPollPageDescription({
        isComplete,
    });
    const imageAlt = createPollSocialImageAlt(normalizedPollTitle, {
        isComplete,
    });
    const imageUrl = createPollPageImageUrl(
        normalizedOrigin,
        normalizedPollSlug,
        isComplete,
    );
    const title =
        normalizedPollTitle ||
        (isComplete ? `Poll results | ${siteName}` : `Poll | ${siteName}`);
