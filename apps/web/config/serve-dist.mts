...
const start = async (): Promise<void> => {
    ...
    const server = http.createServer(async (request, response) => {
        ...
        const pollSocialImageSlug = extractPollSocialImageSlugFromPathname(
            requestUrl.pathname,
        );
        const pollSocialImageVariant =
            extractPollSocialImageVariantFromSearchParams(
        ...
        if (
            ...
            try {
                const pollSocialImageResponse =
                    await createPollSocialImageResponse({
                        apiBaseUrl: seoApiBaseUrl,
                        pollSlug: pollSocialImageSlug,
                        signal: AbortSignal.timeout(5000),
                        variant: pollSocialImageVariant,
                    });

                response.statusCode = pollSocialImageResponse.status;

                Object.entries(pollSocialImageResponse.headers).forEach(
